...
function Register(props) {
   ...
   // const[icon,setIcon]=useState(eyeOff);
    ...
    return (
     ...
     {!isLnameValid ? <span style={{color:'red', fontSize:'12px',position:"relative",right:"30px"}}>{lnameError}</span> : null}
      ...
      <div>
     <input type='password' placeholder='New Password'  name="password"  onChange={(event)=>{handleChange(event)}} value={userDetails.password} autoComplete="off" />
     
     {!isPasswordValid ?<span style={{color:'red', fontSize:'11px',position:"relative",right:"30px"}}>{passwordError}</span> : null}
      </div> 

      <div>
     <input type="password" placeholder='Confirm Password' name="cpassword" onChange={(event)=>{handleChange(event)}} value={userDetails.cpassword} style={{position:"relative",right:17}} autoComplete="off" />
     ...
     <input type="submit" />
      ...
      <p style={{marginTop:"20px"}}>Already have account? </p> <br/> <br/>
   <Link to="/Login" style={{position:"relative",right:"190px" , top:"20px",textDecoration:"none"}}>Login</Link>
        ...
        <div className="col-3  registration-form-container-2">
            <img src="https://thumbs.dreamstime.com/b/registration-hand-pressing-button-interface-blue-background-49410297.jpg" height={500} width={500} />
           </div>
